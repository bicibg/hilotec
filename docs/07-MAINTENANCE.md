...
| Review security audit results | `php artisan security:audit` | The scheduled audit runs automatically every 6 hours, but a manual check lets you see output directly |
...
| Test the deployment process | Run `deploy.sh` on a staging environment | Verify deployments still work correctly |
...
# 8. Deploy to production using the deploy script
./deploy.sh
```
...
storage/
  ...
  quarantine/        -- Files quarantined by SecurityAudit command
```
...
# The deploy.sh script runs "npm ci" and "npm run build" only if needed,
...
| Security audit clean | `php artisan security:audit` | "All clear" | Issues found |
...
| Run security scan | `php artisan security:audit` |
| Run security scan + fix | `php artisan security:audit --fix --notify` |
...
| Deployment failed | 1. Check `deploy.sh` output for errors 2. Check disk space 3. Verify git remote is accessible 4. Check Composer/npm can connect to package registries |
