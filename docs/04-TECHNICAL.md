...
|   Pipeline       |  - SecurityHeaders (adds CSP, HSTS, etc.)
|                  |  - CSRF verification
|                  |  - Session handling
+------------------+
     |
...
|   (outbound)     |  - SecurityHeaders adds HTTP headers
+------------------+
     |
     v
...
|   |-- Console/
|   |   |-- Commands/
|   |       |-- SecurityAudit.php   # Artisan command: php artisan security:audit
|   |
...
|-- .env.production.example         # Production-specific .env settings
```
...
The User model handles authentication. It implements Filament's `FilamentUser` interface, which means it has a `canAccessPanel()` method that controls who can access the admin panel.
...
| `canAccessPanel(Panel)` | method | Returns `true` only if the user's email is in the `ADMIN_EMAILS` env variable |
...
|  (all public pages)  |  Permissions-Policy, etc.
+---------------------+
  |
  v
+---------------------+
...
|  + canAccessPanel()  |  Email must be in ADMIN_EMAILS env
+---------------------+
  |
  v
+---------------------+
|  ThrottleRequests    |  Rate limiting: 60 req/min for admin
|  + ThrottleAdminLogin|  + 5 login attempts/min per IP
+---------------------+
  |
  v
+---------------------+
