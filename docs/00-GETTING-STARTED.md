...
| Branch | Description |
|-------------|-------------|
| `master` | Original dark theme with security hardening. The admin panel requires the `ADMIN_EMAILS` environment variable to be set. |
| `design-v2` | Modern hybrid light/dark design refresh. Security hardening has not been applied to this branch yet. |
...
> **Tip:** On the `design-v2` branch this variable may not be required, but it does no harm to set it anyway.
