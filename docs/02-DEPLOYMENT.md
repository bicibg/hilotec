...
| File | Purpose |
|------|---------|
| `deploy.sh` | Automated deployment script (git pull, clean, build, optimize, permissions) |
| `nginx-security.conf` | Security rules to include in your Nginx server block |
| `php-hardening.ini` | PHP security settings for FPM |
| `.env.production.example` | Production `.env` template with security defaults |
| `public/.htaccess` | Hardened Apache rewrite rules (HTTPS, block PHP in assets, block scanners) |
...
To get A+ on SSL Labs, the HSTS header must be present. The application's `SecurityHeaders` middleware already sends:
...
- [ ] **Admin throttled:** 6+ rapid login attempts return 429
- [ ] **Scanner paths blocked:** `curl -sI https://www.hilotec.com/wp-admin` returns 403, 404, or connection reset
- [ ] **Security audit clean:** `php artisan security:audit` reports no issues
