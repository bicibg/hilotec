...
### 8. Configure Admin Access (master branch only)

> **Note:** This step is only needed on the `master` branch. On `design-v2`, any authenticated user can access the admin panel -- you can skip this step.
...
|--------------|------------------------|--------------------------------------------------|
| `master`     | Dark theme             | Production branch with full security hardening (SecurityHeaders, ThrottleAdminLogin, SecurityAudit, ADMIN_EMAILS, hardened .htaccess, deploy.sh) |
| `design-v2`  | Light/dark hybrid      | Redesign branch with Alpine Precision theme, scroll animations, and animated counters. No security hardening -- any authenticated user can access admin. |
...
On the **master** branch, remember to set `ADMIN_EMAILS=admin@hilotec.com` in your `.env` file. The **`design-v2` branch does not require this** -- it does not implement `FilamentUser` or `canAccessPanel()`, so any authenticated user can access the admin panel.
...
| **403 Forbidden on `/admin`** (`master` branch only; does not apply to `design-v2`) | `ADMIN_EMAILS` is not set in `.env`, or does not include your user's email | Add `ADMIN_EMAILS=admin@hilotec.com` to `.env` and restart the server |
...
cd hilotec
cp .env.example .env
echo "ADMIN_EMAILS=admin@hilotec.com" >> .env   # master branch only; not needed on design-v2
composer install
php artisan key:generate
touch database/database.sqlite
